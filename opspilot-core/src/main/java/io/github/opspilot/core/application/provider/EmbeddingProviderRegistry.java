...
import static io.github.opspilot.core.application.provider.ProviderRegistryContracts.CapabilityKind.EMBEDDING;
import static io.github.opspilot.core.application.provider.ProviderRegistryContracts.failure;

/** Explicit, type-safe registry for embedding providers. */
public final class EmbeddingProviderRegistry {
    private static final String REGISTRY_NAME = "embedding";
    private final Map<String, EmbeddingPort> providers = new LinkedHashMap<>();
    private final Map<String, Set<ProviderCapabilityKey>> capabilities = new LinkedHashMap<>();
    private boolean frozen;

    public synchronized void register(
            String stableProviderId, EmbeddingPort provider, Set<ProviderCapabilityKey> capabilityKeys) {
        ensureMutable(stableProviderId);
        Objects.requireNonNull(provider, "provider");
        ProviderRegistryContracts.validateRegistration(REGISTRY_NAME, EMBEDDING, stableProviderId, capabilityKeys);
    ...
    public synchronized void freeze(Set<ProviderRequirement> requirements) {
        ...
        ProviderRegistryContracts.validateRequirements(REGISTRY_NAME, EMBEDDING, capabilities, requirements);
        frozen = true;
    }

    public synchronized EmbeddingPort require(ProviderCapabilityKey capabilityKey) {
        ensureFrozen(capabilityKey.stableProviderId());
        EmbeddingPort provider = providers.get(capabilityKey.stableProviderId());
