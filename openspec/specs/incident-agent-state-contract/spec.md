...
`IncidentAgentState` MUST 以版本化 JSON 合同保存身份、进度、steps/attempts、Evidence/Hypothesis 引用、预算、循环治理、缺失、失败、报告和取消字段，且字段含义与权威状态所有者一致。

#### Scenario: 受支持版本往返
- **GIVEN** 任一受支持 Schema 版本的完整合法状态
- **WHEN** 状态被序列化后再反序列化
- **THEN** 所有合同字段和值保持等价且状态所有权不改变
