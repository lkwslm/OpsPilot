...
涉及者包括后续实现者、合同/安全审查者、模型与基础设施选型者以及 CI 维护者。真实 Provider 凭证、目标开发机和镜像/模型不可变身份是外部条件，缺失时相关任务必须保持 `BLOCKED`，不能用替代实现宣称通过。License 审查采用尽力而为：已识别结果进入报告，无法识别的组件标记 `WARNING`，不单独阻塞应用跑通。
...
- **风险：真实 Provider、模型或目标开发机暂不可用** → 任务保持 `BLOCKED` 并记录缺失条件；允许先完成不依赖该条件的静态实现，但不得勾选真实门禁。License 元数据无法解析时保留组件清单和警告，不作为单独阻塞条件。
