...
每个阶段的详细内容保留在该阶段自己的 Markdown 文件中，不再拆分二级任务文档。阶段文件中的“建设范围”定义本阶段必须完成的能力边界，“详细实施计划”把范围拆成可排期、可验收的工作包。实施时使用以下规则：

1. 工作包编号固定为 `<阶段>-WP<序号>`，例如 `03-WP04`；包内任务使用 `<工作包>.T<序号>`。编号一旦进入 Issue、PR、测试报告或 Release Manifest，不因任务排序调整而复用。
2. 每个工作包必须明确输入、实施动作、代码/配置落点、依赖、验证和证据。只有实现动作和验证动作同时完成，工作包才可关闭。
3. “代码/配置落点”是预期责任边界，不授权创建设计之外的新 Maven 模块。实际包名可以遵循代码风格，但不得改变第 28 章依赖方向。
4. 阶段内按工作包依赖顺序执行。无依赖的包可以并行开发，但同一状态、Schema、Migration 或 Workflow 的修改必须由一个工作包负责最终集成，避免产生两套事实源。
5. 每个 PR 标注工作包/任务 ID、对应设计章节、机器合同版本、完成的测试以及仍为 `BLOCKED` 的外部条件。跨阶段提前实现只能作为被后续阶段复用的最小基础，不得提前宣称后续门禁完成。
6. 工作包中列出的文件尚不存在时，应在指定模块内创建；文件已经存在时进行增量实现。不得为了匹配路径示例而移动设计已冻结的所有权。

### 工作包完成定义

一个工作包只有同时满足以下条件才算完成：

- 任务清单中的实现项全部落地，且没有未记录的临时分支、Mock/Fake 或静默回退；
- 正常、合法空结果、技术失败、取消/恢复中适用的路径均有自动化测试；
- 合同、状态、权限或事务相关负向测试能够证明错误输入确实被拒绝；
- 从干净环境执行的命令、退出码、报告 URI 和 SHA-256 已记录；
- 日志、Trace、测试报告和 Artifact 已通过 Secret、Ground Truth 和敏感 Prompt 泄漏检查；
- 若实际实现迫使设计边界变化，工作包保持 `BLOCKED`，先提交 ADR 和设计修订，不在代码中消化偏差。

### 阶段内验证顺序

每个阶段统一按以下顺序收敛，后一步不得掩盖前一步失败：

```text
静态合同/配置校验
→ 单元与架构测试
→ PostgreSQL/Provider/Adapter 合同测试
→ Compose 或跨进程集成测试
→ 场景/E2E/性能/安全门禁（适用时）
→ 证据清单和阶段完成审查
```
