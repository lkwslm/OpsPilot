...
10. 任一模型、A2A、Tool、Embedding 或 Rerank 技术链路失败时，不得使用 Mock、固定结果或替代检索保底；有限重试后返回 `FAILED`、结构化错误和可关联的受控日志。
...
| Jaeger | Trace 是 span 图和时序检索，不适合由业务 JPA 表临时复刻；`TraceSearchTool` 使用真实 Trace API | 分布式调用链、错误 span 和耗时定位 | 多一个服务及 Trace 保留成本；首期采样、限期保留，不作为业务真源 |
